...
export async function readContextUsage(
  ...
  try {
    ...
    // Local sessions (Ollama, Atomic Chat) live in the workspace portable
    // store, not the gateway. Count tokens from the persisted message log.
    if (explicitSessionId) {
      ...
      if (localSession) {
        ...
        const model = localSession.model || ''
    ...
    // If the caller asked for a specific session and neither the local store
    // nor the gateway has it, return empty. Falling back to listSessions[0]
    // would make a brand-new chat inherit unrelated context-usage from an old
    // session.
