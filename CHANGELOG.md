...
- **VPS `/docker/hermes-agent-zjya/data/switch-model-internal.sh`** — `case`-Statement um `gemma4:31b` erweitert (sonst würde der Switch mit `Bad model: gemma4:31b` exit 1 abbrechen, selbst wenn der allowlist-Check im Workspace-Code grünes Licht gibt).
- **VPS `/docker/hermes-agent-zjya/data/models.json`** — Picker-Eintrag mit Display-Name "Gemma 4 31B" und `provider: ollama-cloud`. Backups beider VPS-Files unter `*.bak.<unixts>`.
