...
- **"Hermes Updated"-Modal nur 1× pro Release** (`src/components/update-center-notifier.tsx`) — `storeNotes()` löscht den `NOTES_SEEN_KEY` jetzt nur noch, wenn die Notes-ID sich tatsächlich geändert hat. Bisher poppte das Modal bei jedem Page-Refresh erneut, weil `/api/update/status` bei jedem Poll denselben `pendingReleaseNotes`-Payload zurückgibt und `useEffect` darauf gehängt war. Cherry-pick upstream `16bc6e82` (PR #386).

### Fixed — Session-Handling (Upstream PR #350 hand-merged) (2026-05-09)
- **PATCH `/api/sessions` für Local-Sessions** (`src/routes/api/sessions.ts`) — neuer `getLocalSession`-Branch vor dem Gateway-`updateSession()`-Call ruft `updateLocalSessionTitle(sessionKey, label)` aus dem `local-session-store`. Bisher liefen Renames von Ollama- oder Atomic-Chat-Sessions in einen 404 gegen die Gateway, die diese Sessions nicht kennt. Unsere `applyStoredTitles()` + `withDerivedLocalTitles()` für Zero-Fork-Mode bleiben unangetastet — beide Pfade sind komplementär.
- **`/api/session-status` synthetic-key fix** (`src/routes/api/session-status.ts`) — bei `isSyntheticSessionKey(sessionKey)` (transiente `api-*`-Keys) wird jetzt direkt ein Idle-Empty-Payload returnt; vorher fiel der Code auf `listSessions(1, 0)[0].id` zurück und ließ damit eine brand-new-Chat-Session den Context-Usage einer fremden, älteren Session erben. Zusätzlich neuer `getLocalSession`-Branch für Local-Sessions, der `contextPercent/maxTokens/usedTokens` aus `readContextUsage()` zieht. `listSessions`-Import entfernt (unused).
- **`/api/context-usage` accept `sessionKey` alias + Early-Return** (`src/routes/api/context-usage.ts`) — Query-Param-Alias zu `sessionId`; bei `sessionId === 'new' || 'main'` wird der Gateway-Call übersprungen und ein leerer Snapshot returnt.
- **`readContextUsage()` Local-Session-Support + No-Fallback** (`src/server/context-usage.ts`) — neuer Local-Session-Branch zählt Tokens lokal aus `getLocalMessages()` durch `CHARS_PER_TOKEN` gegen `getContextWindow(model)`. **Bug-Fix:** wenn `explicitSessionId` gesetzt aber weder Local noch Gateway das findet, returnt jetzt `emptySnapshot()` statt auf "letzte Session aus listSessions" zurückzufallen — verhindert Context-Vermischung über fremde Sessions.
- **`<SessionItem>` Title-Fallback** (`src/screens/chat/components/sidebar/session-item.tsx:78-91`) — `getSessionDisplayTitle()` fällt jetzt zwischen `derivedTitle` und `'Naming…'` auf `session.title` zurück. Persistierte Local-Session-Titel erscheinen nach Reload sofort statt erst nach Re-Trigger der Auto-Title-Hook.
- **`isAgentSession()` Whitelist statt Blacklist** (`src/hooks/use-agent-view.ts:214-232`) — Agent-Sidebar zeigt jetzt nur noch Sessions mit `kind ∈ {agent, worker, delegate, swarm, subagent}`. Bisher "inclusive by default" → Phantom-Personas wie „Nova — Security Specialist" tauchten während normaler Chat-Streams im rechten Agent-Panel auf, weil normale Chat-Sessions den gleichen Endpoint mit `kind="chat"` und transienten `api-*`-Keys liefern.
